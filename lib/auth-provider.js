...
SmartHomeModel.generateAuthCode = function (username, clientId) {
...
SmartHomeModel.getAccessToken = function (code) {
    ...
    if (new Date(authCode.expiresAt) < Date.now()) {
        console.error('expired code ' + code);
        return false;
    }

    let user = authstore.users[authCode.username];
    if (!user) {
        console.error('could not find user ' + authCode.username);
        return false;
    }
...
SmartHomeModel.getUser = function (username, password) {
    ...
    let user = authstore.users[username];
    if (!user) {
        console.log('not a user', username);
        return false;
    }
...
Auth.registerAuth = function (config, app) {
    setConfig(config);
    ...
    app.get('/oauth', function (req, res) {
        ...
        if (authCode) {
            ...
            return res.send(util.format('auth code: %s', authCode));
        }
    ...
    app.post('/login', function (req, res) {
        ...
        const authCode = SmartHomeModel.generateAuthCode(req.body.username, req.body.client_id);

        if (authCode) {
            console.log('authCode successful ', authCode);
            if (req.body.redirect_uri && req.body.redirect_uri !== 'undefined') {
                return res.redirect(util.format('%s?code=%s&state=%s',
            ...
            return res.send('authCode failed');
        }
    });
...
function setConfig(config) {
    authstore.clients[config.ghome.oauthClientId] = {
        clientSecret: config.ghome.oauthClientSecret,
        uid: 99
    };
    authstore.users = config.ghome.oauthUsers;
}
