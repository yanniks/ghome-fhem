...
//console.log(d);
                ...
                if (connection.fhem.ghome_device && d[0] === connection.fhem.ghome_device.Name) {
                    if (d.length == 3) {
                        if (d[1] === 'no definition')
                            ;//connection.fhem.updateghomeDevice();
...
//console.log( 'value: ' + value );
                ...
                if (connection.fhem.ghome_device) {
                    if (device === 'global') {
                    ...
                    if (device === connection.fhem.ghome_device.Name) {
                        if (reading === 'ghomeMapping' || reading === 'ghomeTypes' || reading === 'echoRooms' || reading === 'fhemIntents'
                            || reading.match(/^ghome.*Level$/)) {
                            connection.fhem.updateghomeDevice();
                            continue;
                        }
                    }
...
FHEM.prototype.updateghomeDevice = function () {
    this.execute('jsonlist2 TYPE=ghome',
        function (result) {
            try {
                var d = JSON.parse(result);
                if (d.totalResultsReturned === 1) {
                    this.ghome_device = d.Results[0];
                    this.log.info('ghome device is ' + this.ghome_device.Name);
                    this.ghome_device.Attributes.genericDeviceType = 'switch';
                    this.xxx = new FHEMDevice(this, this.ghome_device);
                    this.xxx.fromHomebridgeMapping(this.ghome_device.Attributes.ghomeMapping);
                    ...
                    this.ghomeMapping = this.xxx.mappings;
                    this.ghomeTypes = this.ghome_device.Attributes.ghomeTypes;
                    this.echoRooms = this.ghome_device.Attributes.echoRooms;
                    this.fhemIntents = this.ghome_device.Attributes.fhemIntents;
                    this.ghomeConfirmationLevel = this.ghome_device.Attributes.ghomeConfirmationLevel;
                    this.ghomeStatusLevel = this.ghome_device.Attributes.ghomeStatusLevel;
                    delete this.xxx;
                    this.execute('{$defs{' + this.ghome_device.Name + '}->{"ghome-fhem version"} = "' + version + '"}');
                    this.emit('ghome DEVICE', this.ghome_device.Name);
                }// else
                    //this.log.warn('no ghome device found. please define it.');
    ...
    this.execute(cmd,
        function (result) {
            ...
            if (!result.match(/(^| )genericDeviceType\b/)) {
                var cmd = '{addToAttrList( "genericDeviceType:security,ignore,switch,outlet,light,vacuum,blind,thermometer,thermostat,contact,garage,window,lock" ) }';
                this.execute(cmd,
                    function (result) {
...
FHEMDevice(platform, s) {
    ...
    if (s.Readings.volume) {
        this.service_name = 'light';
        this.mappings[CustomUUIDs.Volume] = {
    ...
    if (s.Attributes.subType == 'VacuumCleaner') {
        ...
        this.mappings.Modes = [{
			...
			mode_attributes: {
                name: 'cleaning_mode',
                name_values: [{
                    name_synonym: ['mode', 'modus', 'saugkraft'],
                    lang: 'de'
                }],
                settings: [{
                    setting_name: 'quiet',
                    setting_values: [{
                        setting_synonym: ['leise', 'gering', 'stufe 1', 'geringste stufe'],
                        lang: 'de'
                    }]
				},
				{
                    ...
                    setting_values: [{
                        setting_synonym: ['mittel', 'stufe 2'],
                        lang: 'de'
                    }]
				},
				{
				...
				},
				{
                    setting_name: 'max',
                    setting_values: [{
                        setting_synonym: ['maximum', 'stärkste stufe', 'stufe 4'],
                        lang: 'de'
                    }]
                }],
                ordered: true
    ...
    if (match = s.PossibleSets.match(/(^| )desired-temp(:[^\d]*([^\$ ]*))?/)) {
        ...
        if (s.Readings.actuator)
            this.mappings[CustomUUIDs.Actuation] = {
                reading: 'actuator',
    ...
    if (s.Readings.pressure)
        this.mappings[CustomUUIDs.AirPressure] = {
            name: 'AirPressure',
            reading: 'pressure',
            format: 'UINT16',
            factor: 1
...
//log( util.inspect(s) );
    ...
    this.ghomeName = s.Attributes.ghomeName ? s.Attributes.ghomeName : this.alias;
    ...
    this.ghomeRoom = s.Attributes.ghomeRoom ? s.Attributes.ghomeRoom : this.room;
