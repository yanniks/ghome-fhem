    ...
    for (let d in this.devices) {
        ...
        if (device.mappings.On
            ...
            if (device.mappings.Modes)
                this.addAttributesModes(device, d);
        ...
        d.attributes = {
            availableModes: [{
                ...
                settings: [{
                    setting_name: 'quiet',
                    setting_values: [{
                        setting_synonym: ['quiet', 'leise', 'gering', 'stufe 1', 'geringste stufe'],
                        lang: 'de'
                    }],
                    setting_name: 'balanced',
                    setting_values: [{
                        setting_synonym: ['balanced', 'mittel', 'stufe 2'],
                        lang: 'de'
                    }],
                    setting_name: 'turbo',
                    setting_values: [{
                        setting_synonym: ['turbo', 'stark', 'stufe 3'],
                        lang: 'de'
                    }],
                    setting_name: 'max',
                    setting_values: [{
                        setting_synonym: ['max', 'maximum', 'stärkste stufe', 'stufe 4'],
                        lang: 'de'
                    }]
                }],
                ordered: true
...
var queryModes = function (device, d) {
    if (device.service_name === 'vacuum') {
        const currentMode = device.fhem.cached(device.mappings.Modes.informId);
        devices[d.id].currentModeSettings = {
            cleaning_mode: currentMode
        }
    }
}
...
var handleControlSetMode = function (cmd, event) {
    ...
    cmd.devices.forEach((d) => {
        ...
        current = device.fhem.cached(device.mappings.Modes.informId);

        device.command(device.mappings.Modes, event.params.updateModeSettings.cleaning_mode);
        deviceIds.push(d.id);
    });
    ...
    return [{
        states: {
            currentModeSettings: {
                cleaning_mode: event.params.updateModeSettings.cleaning_mode
            }
        },
        status: 'success',
        ids: deviceIds
