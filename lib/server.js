        ...
        cmd.execution.forEach((exec) => {
            ...
            switch (requestedName) {
                ...
                case REQUEST_STARTSTOP:
                    responses.push(...handleEXECUTEStartStop(this)(cmd, event.params.start));
                    break;

                case REQUEST_PAUSEUNPAUSE:
                    responses.push(...handleEXECUTEPauseUnpause(this)(cmd, event.params.pause));
                    break;
...
var handleEXECUTEPauseUnpause = function(cmd, pausebool) {
    let deviceIds = [];
    ...
    cmd.devices.forEach((d) => {
        ...
        let pauseCmd = device.mappings.StartStop.cmdUnpause;
        if (pausebool)
            pauseCmd = device.mappings.StartStop.cmdPause;

        device.command(pauseCmd, '');
        deviceIds.push(d.id);
    });
...
var handleEXECUTEStartStop = function(cmd, startbool) {
    let deviceIds = [];
    ...
    cmd.devices.forEach((d) => {
        ...
        let onOffCmd = device.mappings.StartStop.cmdOff;
        if (startbool)
            onOffCmd = device.mappings.StartStop.cmdOn;

        device.command(onOffCmd, '');
        deviceIds.push(d.id);
    });
