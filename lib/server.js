...
// Look for the configuration file
    ...
    if (typeof config.ghome.oauthClientID !== 'object')
        config.ghome.oauthClientID = [config.ghome.oauthClientID];
...
Server.prototype.run = function () {
    ...
    this.roomOfIntent = {};
    if (this._config.ghome.applicationId)
        for (var i = 0; i < this._config.ghome.applicationId.length; ++i) {
            var parts = this._config.ghome.applicationId[i].split(':', 2);
            if (parts.length == 2) {
                this.roomOfIntent[parts[0]] = parts[1].toLowerCase();
                this._config.ghome.applicationId[i] = parts[0];
            }
        }
    if (this._config.ghome.oauthClientID)
        for (var i = 0; i < this._config.ghome.oauthClientID.length; ++i) {
            var parts = this._config.ghome.oauthClientID[i].split(':', 2);
            if (parts.length == 2) {
                this.roomOfIntent[parts[0]] = parts[1].toLowerCase();
                this._config.ghome.oauthClientID[i] = parts[0];
            }
        }
...
var verifyToken = function (event, callback) {
    ...
    if (token === accepted_token && Date.now() < expires) {
        ...
        const url = "https://api.amazon.com/auth/O2/tokeninfo?access_token=" + token.replace('|', '%7C');
        require('https').get(url, function (result) {
            ...
            result.on('end', function () {
                try {
                    ...
                    } else if (!this._config.ghome.oauthClientID || this._config.ghome.oauthClientID.indexOf(parsedData.aud) >= 0) {
        ...
        if (event.session.application && event.session.application.applicationId
            && this._config.ghome.applicationId.indexOf(event.session.application.applicationId) >= 0) {
            handler.bind(this)(event, callback);

        } else if (event.session.application && event.session.application.applicationId) {
            log.error(this._config.ghome.applicationId);
        ...
        cmd.execution.forEach((exec) => {
            ...
            switch (requestedName) {
                ...
                case REQUEST_SET_PERCENTAGE :
                    responses.push(...handleControlSetPercentage.bind(this)(event));
                    break;
...
var handleQuery = function (input) {
    ...
    input.payload.devices.forEach((d) => {
        ...
        // If there is a current temperature, we probably have a thermostat
        if (device.fhem.cached(device.mappings.CurrentTemperature.informId)) {
            ...
            const humidity = parseFloat(device.fhem.cached(device.mappings.CurrentRelativeHumidity.informId));
            devices[d.id] = {
                online: true,
                thermostatMode: desiredTemp > currentTemp ? 'heat' : 'cool',
                thermostatTemperatureSetpoint: desiredTemp,
                thermostatTemperatureAmbient: currentTemp,
...
var handleControlSetTargetTemperature = function (cmd, targetTemperature) {
    ...
    return [{
        states: {
            thermostatMode: targetTemperature > current ? 'heat' : 'cool',
