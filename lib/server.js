    ...
    input.payload.commands.forEach((cmd) => {
        cmd.execution.forEach((exec) => {
            ...
            switch (requestedName) {
                ...
                case REQUEST_LOCATE:
                    responses.push(...handleEXECUTELocate.bind(this)(cmd, event));
                    break;
                    
                case REQUEST_STARTSTOP:
                    responses.push(...handleEXECUTEStartStop(this)(cmd, event.params.start ? 1 : 0));
                    break;

                case REQUEST_PAUSEUNPAUSE:
                    responses.push(...handleEXECUTEPauseUnpause(this)(cmd, event.params.pause ? 1 : 0));
                    break;

                case REQUEST_FANSPEED:
                    responses.push(...handleEXECUTESetFanSpeed(this)(cmd, event.params.fanSpeed));
                    break;

                case REQUEST_FANSPEEDREVERSE:
                    responses.push(...handleEXECUTEReverse(this)(cmd, event.params.reverse));
                    break;

                //action.devices.traits.Modes: COMMANDS
                case REQUEST_SET_MODE:
                    responses.push(...handleEXECUTESetMode.bind(this)(cmd, event));
                    break;
...
var handleQUERY = function (input) {
    ...
    input.payload.devices.forEach((d) => {
        ...
        if (device.mappings.FanSpeed) {
            ...
            devices[d.id].currentFanSpeedSetting = (parseFloat(device.fhem.cached(device.mappings.FanSpeed.informId)) < 50)? 'Low':'High';
        }
...
var handleEXECUTESetFanSpeed = function(cmd, speedname) {
    ...
    cmd.devices.forEach((d) => {
        ...
        device.command(device.mappings.FanSpeed, device.mappings.FanSpeed['speed_' + speedname]);
        deviceIds.push(d.id);
    });
